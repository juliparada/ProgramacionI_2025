...
namespace PrimerProyectoCsharp
{
    public partial class FrmPeriodos : Form
    {
        ...
        {
            // Ejemplo dentro del método de guardado
            if (string.IsNullOrWhiteSpace(txtNombrePeriodo.Text))
            {
                MessageBox.Show("El nombre del periodo es obligatorio.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
