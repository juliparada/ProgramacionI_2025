    ...
    public partial class FrmNotas : Form
    {
        ...
        private void actualizarDs()
        {
            ...
            objConexion.objAdaptador = new SqlDataAdapter("SELECT alumnos.nombre, dnotas.idDetalle, dnotas.idNota, dnotas.idMateria, " +
                 ...
                 "WHERE notas.idPeriodo=" + idPeriodo + " AND dnotas.idMateria=" + idMateria, selectConnection: objConexion.objConexion);
            objConexion.objAdaptador.Fill(objNotas, "notasAlumnos");
        }
        private void actualizarGrid()
        {
            actualizarDs();
            detallesDataGridView.DataSource = objNotas;
            detallesDataGridView.DataMember = "notasAlumnos";
        }
        ...
        private void frmNotas_Load(object sender, EventArgs e)
        {
            ...
            this.detallesTableAdapter.Filldetalles(this.db_academicaDataSet.detalles);
        ...
        {
            int nFilas = detallesDataGridView.Rows.Count;
            for (int i = 0; i < nFilas; i++)
            {
                double lab1 = 0, lab2 = 0, parcial = 0;
                int idDetalle = int.Parse(detallesDataGridView.Rows[i].Cells["idDetalle"]?.Value?.ToString() ?? "0");

                lab1 = double.Parse(detallesDataGridView.Rows[i].Cells["lab1"]?.Value?.ToString() ?? "0");
                lab2 = double.Parse(detallesDataGridView.Rows[i].Cells["lab2"]?.Value?.ToString() ?? "0");
                parcial = double.Parse(detallesDataGridView.Rows[i].Cells["parcial"]?.Value?.ToString() ?? "0");
